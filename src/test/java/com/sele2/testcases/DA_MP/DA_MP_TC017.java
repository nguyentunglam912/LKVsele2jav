...
public class DA_MP_TC017 extends TestBase {
	...
	public void DA_MP_TC017_CannotDeleteOverviewPage() {
		...
		Log.info("VP: Check confirm message \"Are you sure you want to remove this page?\" appears");
		...
		Log.info("Step 9: Go to the children page");
		homePage.goToPage(pagePath2);
		
		Log.info("Step 10: Click \"Delete\" link");
		Log.info("VP: Check confirm message \"Are you sure you want to remove this page?\" appears");
		...
		Log.info("Step 12: Click on parent page");
		homePage.goToPage(pagePath1);
		...
		Assert.assertFalse(homePage.isDeleteMenuDisplayed());
	}
	
	@AfterMethod
	private void cleanUp(ITestResult result) {
		if (result.getStatus() == ITestResult.SUCCESS) {
			homePage.deleteAllPagesFromMenu(pagePath1);
		}
	}
